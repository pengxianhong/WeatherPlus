...
public class SunTrailView extends View {

    private static final String TAG = "SunTrailView";
    ...
    public SunTrailView(Context context) {
        super(context);
        init(null);
    }

    public SunTrailView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.SunTrailView);
        mainColor = array.getColor(R.styleable.SunTrailView_svMainColor, 0x67B2FD);
        int trackColor = array.getColor(R.styleable.SunTrailView_svTrackColor, 0x67B2FD);
        int sunColor = array.getColor(R.styleable.SunTrailView_svSunColor, 0x00D3FE);
        svSunSize = array.getDimension(R.styleable.SunTrailView_svSunRadius, 10);
        float svTextSize = array.getDimension(R.styleable.SunTrailView_svTextSize, 18);
        textOffset = array.getDimension(R.styleable.SunTrailView_svTextOffset, 10);
        svPadding = array.getDimension(R.styleable.SunTrailView_svPadding, 10);
        svTrackWidth = array.getDimension(R.styleable.SunTrailView_svTrackWidth, 3);
        array.recycle();
        ...
        mDashPathEffect = new DashPathEffect(new float[]{15, 15}, 0);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isDraw) {
            ...
            // 渐变路径
            Shader mPathShader = new LinearGradient(mWidth / 2, svPadding, mWidth / 2, endY,
    ...
    public void motionAnimation() {
        if (valueAnimator == null) {
            ...
            animator.setDuration((long) (10000 * (mProgress - mCurrentProgress)));
