...
public abstract class BaseDashboardView extends View {
    ...
    // 默认刻度和文字（刻度和文字要一一对应）
    private final static int[] DEFAULT_CALIBRATION_NUMBER = new int[]{0, 50, 100, 200, 300, 500};
    private final static String[] DEFAULT_CALIBRATION_BETWEEN = new String[]{"优", "良", "轻度污染", "中度污染", "重度污染"};
    // 默认刻度数量
    ...
    // 数值画笔属性
    private final static float DEFAULT_VALUE_TEXT_SIZE = 35f;
    private final static int DEFAULT_VALUE_TEXT_COLOR = Color.WHITE;
    // 数值等级画笔属性
    private final static float DEFAULT_VALUE_LEVEL_TEXT_SIZE = 20f;
    ...
    public void setValue(int value, boolean isAnim, boolean reset) {
        value = value < mMin ? mMin : Math.min(value, mMax);
        //计算进度需要转动的角度
